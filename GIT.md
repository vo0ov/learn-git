...
git reset --hard BRANCH # Прировняет BRANCH к текущей ветке (BRANCH не будет изменена, а текущая сбросится до состояния BRANCH)
```
...
git push origin BRANCH --force # -f для с нуля скопирует из локальной в удалённую
git remote -v # Показывает подключенные удалённые репозитории
```